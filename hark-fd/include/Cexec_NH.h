 ...
 */
// ---------------------------------------------------------------------
#ifndef	Cexec_NH_h_defined
#define	Cexec_NH_h_defined
// ---------------------------------------------------------------------
...
public:
	...
	*/
	...
	/**
	 \breif \~Japanese 最近傍位置のデータインデックスを取得(Neighbor利用)
	 ...
	 \param level \~English Level of Detection
	 \~Japanese 検索レベル。0の場合、登録されたneighborすべてについて検索。>=1の場合、level位までのneighborについて検索する
	*/
	HA_Integer GetNearestPos(MICARY_Location micary_loCur, MICARY_LocationArray *micary_laDB, int level=0) ;

	/**
	 \breif \~Japanese 最近傍位置のデータインデックスを取得(座標計算)
 	 \return \~English Output index 	
	 \param micary_loCur \~English Current source position
	 \~Japanese 現在の音源座標
	 \param micary_laDB \~English Database source position array
	 \~Japanese 検索元音源位置座標データベース
	*/
	HA_Integer GetNearestPosByCoord(MICARY_Location micary_loCur, MICARY_LocationArray *micary_laDB) ;

	/**
	 \breif \~Japanese TFデータベース中で最近傍位置の位置IDを取得(座標計算による)
 	 \return \~English Output PosID
	 \param micary_loCur \~English Current source position
	 \~Japanese 現在の音源座標
	*/
	HA_Id GetPosIdFromTFDBByCoord(MICARY_Location micary_loCur);
	...
	*/
	...
	//Changed by N.Tanaka 2014/08/27
	/**
	 ...
	 \param fThred_dist \~English The threshold distance[mm] for check source position.
	 \~Japanese 音源位置確認のための許容角度[mm]
	*/
	bool SetPrmUpdate(int iUpdateMTF_CONJ,int iUpdateMW,float fThred_dist);
...
private:
	...
	CHarkFileIO cHark ;	// TFファイル用読込み
	CHarkFileIO cHarkW;	// W書き込み用
	CHarkFileIO cHarkTmp;	// Tmp用
...
　 　 **/	
	...
	/**
	 \brief \~Japanese 2点のユークリッド距離を求める
	 \return \~English Calicurate euclidean distance
	 \param laRefPos \~English Source location (MICARY_Location type)
	 \~Japanese 音源ベクトル
	 \param laObjPos \~English Source location (MICARY_Location type)
	 \~Japanese 音源ベクトル
	*/
	HA_Float _EuclidDistance(MICARY_Location laRefPos,MICARY_Location laObjPos);

	/**
	 \brief \~Japanese 2点の位置がThreshold_lengthで設定された範囲内であるかの判定(Neighbor利用)
	...
	*/	
	...
	/**
	 \brief \~Japanese 2点の位置がThreshold_lengthで設定された範囲内であるかの判定(座標計算)
	 \return \~English check code(true is in range ,false is out of range)
	 \param laRefPos \~English reference(saved) position
	 \~Japanese 参照位置
	 \param laObjPos \~English objective position
	 \~Japanese 目的位置
	*/	
	bool _IsThreshPosByCoord(MICARY_Location laRefPos,MICARY_Location laObjPos);
	...
	*/
	...
	/**
	\brief \~Japanese POSによる確認時の許容範囲 (距離[m])
	\~English　The threshold dist[m] when it checks source change by POS. 
	*/
	HA_Float m_fThred_dist;	 
