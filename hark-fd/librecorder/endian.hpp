...
inline void swap_endian(void* p);

template <>
inline void swap_endian<2>(void* p) {
    char* pb = (char*)p;
    std::swap(pb[0], pb[1]);
}

template <>
inline void swap_endian<4>(void* p) {
    char* pb = (char*)p;
    std::swap(pb[0], pb[3]);
    std::swap(pb[1], pb[2]);
}
...
template <>
inline void swap_endian<8>(void* p) {
    char* pb = (char*)p;
    std::swap(pb[0], pb[7]);
    std::swap(pb[1], pb[6]);
    std::swap(pb[2], pb[5]);
...
inline Type swap_endian(Type v) {
    Type result = v;
...
inline void swap_endian(InputIterator first, InputIterator last) {
