...
public:
      ...
      rxxsID(-1), brxxsID(false), rxxnID(-1), brxxnID(false) 
  {
    ...
    localizer.reset(new LocalizationMUSIC(nb_channels, samplingRate,
                                          length, length, maxnum_out_peaks));
    cout << "reading A matrix\n";
    localizer->ReadTransferFunction(const_cast<char *>(FilenameAMatrix.c_str()), ch_selection);
    cout << "done\n";
    localizer->SetNumSource(num_source);
    localizer->SetUsedFrequency(lower_bound, upper_bound);
    ptfname = FilenameAMatrix;

    nid = localizer->GetSearchId().size();

    tf_info = Vector<ObjectRef>::alloc(0);
    for (unsigned int i = 0; i < localizer->GetSearchId().size(); i++) {
      RCPtr<Source> src(new Source);
      src->id = i;
      src->power = 0.0;
      src->x[0] = localizer->GetSearchId()[i][0];
      src->x[1] = localizer->GetSearchId()[i][1];
      src->x[2] = localizer->GetSearchId()[i][2];
  ...
  void calculate(int output_id, int count, Buffer &out)
  {
    ...
    RCPtr<Vector<float > > spectrum(new Vector<float >(nid));    
    if(spectrumID != -1){    
    ...
    if(spectrumbinID != -1){    
      ...
      (*spectrumbin).vsize.resize(2);
      (*spectrumbin).vsize[0] = pslength;
      (*spectrumbin).vsize[1] = nid;
      (*spectrumbin).data.resize(pslength * nid);
    ...
    for (int i = 0; i < n_result; i++) {
      const int found_id = localizer->GetFoundId()[i];
      if (is_debug) {
	printf("%d: %7.2f %7.2f %7.2f %7.2f\t", i,
	       (float)(localizer->GetSearchId()[found_id][0]),
	       (float)(localizer->GetSearchId()[found_id][1]),
	       (float)(localizer->GetSearchId()[found_id][2]),
	       localizer->GetP(localizer->GetFoundId()[i]));
      }
      RCPtr<Source> src(new Source);
      src->id = 0;
      src->power = localizer->GetP(found_id);
      src->x[0] = localizer->GetSearchId()[found_id][0];
      src->x[1] = localizer->GetSearchId()[found_id][1];
      src->x[2] = localizer->GetSearchId()[found_id][2];
      src->tfindex[0] = found_id;
      src->tfindex[1] = 0;
      src->tfindex[2] = 0;
      src->tfindex_max[0] = localizer->GetSearchId().size() - 1;
      src->tfindex_max[1] = 0;
      src->tfindex_max[2] = 0;
      src->tfindex_min[0] = 0;
      src->tfindex_min[1] = 0;
      src->tfindex_min[2] = 0;
      src->compare_mode = Source::TFINDEX;	// 2015/02/25 added flag
	...
	for (int iid = 0; iid < nid; iid++) {
	  (*spectrumbin).data[iid + bin * nid] = localizer->GetP_bin(bin,iid);
	}    
      }
    }
    ...
    if(tfnameID != -1){
      ...
      if(ptfname != tfname){
	localizer->ReadTransferFunction(const_cast<char *>(tfname.c_str()), ch_selection);
	nid = localizer->GetSearchId().size();
      }
      ptfname = tfname;
    }
