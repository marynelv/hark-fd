 ...
 *
 * @parameter_name UPDATE_ACCEPT_DISTANCE
 * @parameter_type float
 * @parameter_value 300.0
 * @parameter_description Distance allowance to switch separation matrix in [mm]. available when when UPDATE_METHOD_W is POS or ID_POS. [default: 300.0]
 *
...
public:
	...
	if(parameters.exist("SS_SCAL")){
	    ...
	    */
	    ...
	    if(parameters.exist("UPDATE_ACCEPT_DISTANCE")){
			fUpdate_dist = dereference_cast<float>(parameters.get("UPDATE_ACCEPT_DISTANCE"));
	    }
	    else{
			fUpdate_dist = 300.0 ;
	    }
	    m_cGHDSSExec.SetPrmUpdate(iUpdate_TF_CONJ, iUpdate_W, fUpdate_dist) ;
...
//		cout << "Frame" << count << ": Src Num=" << iNVecSrc << ":";	// TEST
		for (i=0 ; i < iNVecSrc ; i++){
			...
			// 2014/8/27 deleted by N.Tanaka 角度算出は不要
			/*
			//角度算出はDEGのみ
			...
			*/
			...
			// tfposid情報の追加
			// tfposidはMUSICから発信される
//			locaInputSrcPos.micary_Location[i].tfposid = m_cGHDSSExec.GetPosIdFromTFDBByCoord(locaInputSrcPos.micary_Location[i]);
			// 2015/02/25 changed by N.Tanaka ConstantLocalizationによる座標指定対策
//			locaInputSrcPos.micary_Location[i].tfposid = src->tfindex[0];
			if (src->compare_mode == Source::TFINDEX){
				locaInputSrcPos.micary_Location[i].tfposid = src->tfindex[0];
//cout << "Use Source::TFINDEX ";	// TEST
			}
			else {
				locaInputSrcPos.micary_Location[i].tfposid = m_cGHDSSExec.GetPosIdFromTFDBByCoord(locaInputSrcPos.micary_Location[i]);
//cout << "Use Source::DEG ";	// TEST
			}
//			cout << src -> id << "-" << locaInputSrcPos.micary_Location[i].tfposid << ", ";	// TEST
...
*/			
		...
		if(m_bFlgIni == true){
			...
			*/
		    ...
		    // TEST
//		    cout << "Input Pos@GHDSS: ";
//		    for (i=0 ; i<locaSrcPos.ha_numLocation ; i++){
//		        cout << locaSrcPos.micary_Location[i].tfposid << ", ";
//		    }
//		    cout << endl;
	...
	for (i = 0 ; i < locaDest->ha_numLocation ; i++){
		for (k = 0 ; k < locaSrc->ha_numLocation ; k++){
			if (locaDest->ha_idLocation[i] == locaSrc->ha_idLocation[k]){
				...
				// TFDB中の位置ID
				locaDest->micary_Location[i].tfposid = locaSrc->micary_Location[k].tfposid ;
				break;
			}
		}
	}
