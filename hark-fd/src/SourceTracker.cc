 ...
 *
 * @parameter_name MIN_TFINDEX_INTERVAL
 * @parameter_type int
 * @parameter_value 3
 * @parameter_valid COMPARE_MODE=TFINDEX
 * @parameter_description Source interval threshold in degree. When the angle between a localization result and a source is smaller than MIN_TFINDEX_INTERVAL, the same ID is given to the localization result. [default: 3]
 *
...
public:
      ...
      paramsID(-1), parammapID(-1)
  {
    ...
    if(cmpmode == "DEG"){
      ...
      compare_mode = Source::TFINDEX;
      MinTFIndexInterval = dereference_cast<int>(parameters.get("MIN_TFINDEX_INTERVAL"));
    }
    else{
      throw new NodeException(NULL, "Unknown COMPARE_MODE. This must be DEG/TFINDEX",
                              __FILE__, __LINE__);
    }
    cerr << "compare_mode " << cmpmode << "(" << compare_mode << ")" << endl;
