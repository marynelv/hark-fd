...
class CMDivideEachElement;

DECLARE_NODE(CMDivideEachElement);
/*Node
 *
 * @name CMDivideEachElement
...
class CMDivideEachElement : public BufferedNode {
  int cmaID;
  int cmbID;
  int operationID;
  int outputcmID;
...
public:
  CMDivideEachElement(string nodeName, ParameterSet params)
  ...
  void calculate(int output_id, int count, Buffer &out)
  {
	...
	throw new NodeException(this, string("CMDivideEachElement. Incorrect correlation matrix size."), __FILE__, __LINE__);
      if (!(cma->nrows() == cmb->nrows() && cma->ncols() == cmb->ncols())) {
	throw new NodeException(this, string("CMDivideEachElement. Size mismatched between CMA and CMB."), __FILE__, __LINE__);
      }
      Rxx.resize(pslength);
    ...
    if (typeid(*operation) == typeid(Bool)) { 
      ...
      throw new NodeException(this, string("Input type is not appropriate in CMDivideEachElement."), __FILE__, __LINE__); 
    } 
    ...
    if((operation_flag == 1)||(first_frame_execution)){    
      ...
      if(enable_debug) printf("Count[%d] : CMDivideEachElement Executed\n", count);
  ...
  //IN_ORDER_NODE_SPEEDUP(CMDivideEachElement);
};
