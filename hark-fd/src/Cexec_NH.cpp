							...
							int iFlgSm,const char *filename_sm)
{
	...
	if(iFlgTf == 1){	// Read data file
		...
		// Neighborの計算
		if (filename_tf){
			cHark.CalcNeighbors(filename_tf, m_fThred_dist);	// m_fThred_distはユークリッド距離[m]
		}
	}
	else{
	...
	if(this->m_bFlgFixedNoise){
		...
		if(m_iFormat_FixedNoise == __FILE_FORMAT_HARK){
			if(!cHarkTmp.LoadPositionData(m_file_FixedNoise,&locaSrcFixedNoise,__FILE_TYPE_NOISE)){
...
int Cexec ::ReadMicPos()
{
	...
	if(m_iFormat_MicPos == __FILE_FORMAT_HARK){
		cout << "File open as HARK = " << m_file << endl ; 
	  	if(!cHarkTmp.LoadPositionData(m_file,&locaMic,__FILE_TYPE_MIC)){
...
int Cexec :: InitTFMatrix(MICARY_PrmFrame *psPrmFrame)
{
	...
	iUpper_f = m_psSepMat->micary_smSeparation.micary_iaFreqLinex.ha_numValue +iLower_f -1 ;
		...
		if(!this->m_psTFDB){	//Initialフレーム処理（データベースﾌｧｲﾙの読み込み)
			...
			// MUSICから発信されるため、ここで再計算する必要はない
			// すでに入力した音源情報にID情報を付加する
/*			
			for(i = 0 ; i < m_psSepMat->micary_tdTFDB.micary_locaSource.ha_numLocation ; i ++){
				// TFDB中のインデックス
				m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[i].tfposid = this->GetPosIdFromTFDBByCoord(m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[i]) ;
//				cout << "SrcPos" << i << "@Cexec :: InitTFMatrix" << m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[i].tfposid << endl;	// TEST
			}
*/			
			...
			*/
			...
			// 2014/8/27 deleted by N.Tanaka Neighbor利用のため、付加情報は必要なし
			/*
			//音源の付加情報（ファイルにない情報）
		...
		*/
		...
		for(ii = 0 ; ii < iNch ; ii++){
			...
			// 2015/03/04 added by N.Tanaka #1823
			m_psSepMat->micary_tdTFDB.micary_locaMic.micary_Location[ii].tfposid = m_psTFDB->micary_locaMic.micary_Location[ii].tfposid ;
	...
	if(this->m_iFlgsm == 0){	// Read TF
		...
		if(!m_psSMDB){	//Initial frame process
			...
			if(m_iFormat_InitW == __FILE_FORMAT_HARK){
				if(cHarkW.ReadHeader(file.s,psSMDBHead) == 0) //ファイルのヘッダ情報を取得
				{
	...
	for(int i = 0 ; i < m_psSMDB->micary_smSeparation.micary_iaFreqLinex.ha_numValue ; i++){
	  m_psSepMat->micary_smSeparation.micary_iaFreqLinex.ha_iValue[i] =  m_psSMDB->micary_smSeparation.micary_iaFreqLinex.ha_iValue[i]  ;
		...
		if(Cexec::GetNearestSepMatrixFromDB(m_psSepMat,m_psSMDB) == 0){
			// 2014/11/28 changed by N.Tanaka 初期値使用できない場合はTFから計算
			// m_iFlgtf = 0 ;
			// m_bFlgFixedNoise = false ;
			//m_iFlgsm = 0 ;
			// return 0;
			cout << "Cannot found sources from InitW at 1st step. GHDSS uses the separation matrix calculated by TFCONJ." << endl;
			if(MakeWMatGHDSS(m_psSepMat->micary_tdTFDB,&m_psSepMat->micary_smSeparation) == 0){
				std ::cout << "Fail to initialize sepatation matrix." << std::endl ;
				m_iFlgtf = 0 ;
				m_bFlgFixedNoise = false ;
...
//cout << "this->m_iFlgsm == 1 / Cexec :: LoadInitialSepMatrix" << endl;	// TEST
		//必要な情報を取り出す
		if(Cexec::GetNearestSepMatrixFromDB2(m_psSepMat,m_psSMDB,iSrcIndex) == 0){
			// 2014/11/28 changed by N.Tanaka TF計算による初期化			
			// m_iFlgtf = 0 ;
			// m_bFlgFixedNoise = false ;
			// m_iFlgsm = 0 ;
			// return 0;
			cout << "Cannot found source-index" << iSrcIndex <<  " from InitW. GHDSS uses the separation matrix calculated by TFCONJ." << endl;
			if(MakeWMatGHDSS2(m_psSepMat->micary_tdTFDB, &m_psSepMat->micary_smSeparation, iSrcIndex) == 0){
				std ::cout << "Fail to initialize sepatation matrix." << std::endl ;
				m_iFlgtf = 0 ;
				m_bFlgFixedNoise = false ;
...
int Cexec :: AssignSrcPos(MICARY_LocationArray locaSrcPos)
{
	...
	for(n = 0 ; n < locaSrcPos.ha_numLocation ; n ++){
		...
		// TFDB中のインデックス
		m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[n].tfposid = locaSrcPos.micary_Location[n].tfposid ;
	}
...
int Cexec::ReadSepMatrix(MICARY_PrmFrame psPrmFrame)
{
	...
	if(m_iFormat_InitW == __FILE_FORMAT_HARK){
		...
		if(!cHarkW.ReadInfo(SMfile.s,psSMDBHead,
	...
	// 2014/8/27 deleted by N.Tanaka Neighbor情報利用のため、付加情報は不要
	/*
		//add by Morio 2013/01/20 角度情報を格納
		//角度算出の原点は(0,0,0)とする
		float pi = acos(-1.0) ;
...
// --------------------------------------------------------------------------------
// 2点のユークリッド距離を求める
HA_Float Cexec::_EuclidDistance(MICARY_Location laRefPos,MICARY_Location laObjPos)
{
	float fdist;
	
	fdist = pow(laRefPos.ha_fX - laObjPos.ha_fX, 2.0)
			+ pow(laRefPos.ha_fY - laObjPos.ha_fY, 2.0)
			+ pow(laRefPos.ha_fZ - laObjPos.ha_fZ, 2.0);
	fdist = sqrt(fdist);
	
	return ((HA_Float)fdist);
}
// --------------------------------------------------------------------------------
// 最近傍位置のデータインデックスを取得(Neighbor利用)
HA_Integer Cexec::GetNearestPos(MICARY_Location micary_loCur, MICARY_LocationArray *micary_laDB, int level)
{
	int i, k;
	int iCurIndex;
 ...
 //cout << "(PosID=" << micary_loCur.tfposid << "@GetNearestPos)" << endl; // TEST
	iCurIndex = cHark.GetPosIndex(micary_loCur.tfposid);
	if (iCurIndex < 0 || !cHark.m_pPrinbrs){
		return (GetNearestPosByCoord(micary_loCur, micary_laDB));
	}

	// 検索レベルの指定
	int iNumNeighbors = (level > 0) ? level : cHark.m_pPrinbrs->numNeighbors[iCurIndex];
	
	// micary_loCurのposidに対応するneighborリストを取得し、リスト上位からmicary_laDBを検索する
	// 見つかったらインデックスを返す
//	for (i=0 ; i < cHark.m_pPrinbrs->numNeighbors[iCurIndex] ; i++){	// i==0は本人
	for (i=0 ; i < iNumNeighbors ; i++){	// i==0は本人
		for (k=0 ; k < micary_laDB->ha_numLocation ; k++){
			if (cHark.m_pPrinbrs->neighbors[iCurIndex][i] == micary_laDB->micary_Location[k].tfposid){
//cout << "return "<< k <<" @GetNearestPos)"  << endl; // TEST
				return ((HA_Integer)k);
			}
		}
	}
	
 //cout << "return __INVALID_SRC_ID @GetNearestPos)"  << endl; // TEST
	return __INVALID_SRC_ID;
}
// --------------------------------------------------------------------------------
// 最近傍位置のデータインデックスを取得(座標計算)
HA_Integer Cexec::GetNearestPosByCoord(MICARY_Location micary_loCur, MICARY_LocationArray *micary_laDB)
{
	int i;
	float fmin, fdisttmp;
	int index;
	if (!micary_laDB){
 //cout << "return __INVALID_SRC_ID @GetNearestPosByCoord)"  << endl; // TEST
		return __INVALID_SRC_ID;
	}
	if (micary_laDB->ha_numLocation < 1){
 //cout << "return __INVALID_SRC_ID @GetNearestPosByCoord)"  << endl; // TEST
		return __INVALID_SRC_ID;
	}

	index = 0;
	fmin = _EuclidDistance(micary_loCur, micary_laDB->micary_Location[index]);
	
	for (i=1 ; i < micary_laDB->ha_numLocation ; i++){
		fdisttmp = _EuclidDistance(micary_loCur, micary_laDB->micary_Location[i]);
		if (fdisttmp < fmin){
			fmin = fdisttmp;
			index = i;
		}
 ...
 //cout << "return "<< index <<" @GetNearestPosByCoord)"  << endl; // TEST
	return ((HA_Integer)index);
}
// --------------------------------------------------------------------------------
// TFデータベース中で最近傍位置の位置IDを取得(座標計算による)
HA_Id Cexec::GetPosIdFromTFDBByCoord(MICARY_Location micary_loCur)
{
	HA_Id idPos;
	idPos = cHark.GetNearestPosID(micary_loCur);
	return (idPos);
}
// --------------------------------------------------------------------------------
...
int Cexec::GetNearestSepMatrixFromDB2(MICARY_SavedSeparation *OutSM, MICARY_SMDatabase *SMDB, int idsrc)
{
	...
	// TEST
//	if (iDBpos == __INVALID_SRC_ID){
//		cout << "Not found @ Cexec::GetNearestSepMatrixFromDB2" << endl;
//		cout << "Search Pos: " << OutSM->micary_tdTFDB.micary_locaSource.micary_Location[idsrc].ha_fX << ", "
//			<< OutSM->micary_tdTFDB.micary_locaSource.micary_Location[idsrc].ha_fY << ", "
//			<< OutSM->micary_tdTFDB.micary_locaSource.micary_Location[idsrc].ha_fZ << ", "
//			<< OutSM->micary_tdTFDB.micary_locaSource.micary_Location[idsrc].tfposid << endl;
//	}
	...
	*/
	if(iCount != 1){
//		cout << "Cannot find the same number of TF data.";
		cout << "Cannot find the separation matrix from DB." << endl;
		return 0 ; //音源分見つからなければエラー
	}
	else {
...
// ----------------------------------------------------------------------------------
bool Cexec::SetPrmUpdate(int iUpdateMTF_CONJ,int iUpdateMW,float fThred_dist)
{
	// INITW作成方法 = CALC時はUpdateMethod->ID
	m_iUpdateMethodTF_CONJ = iUpdateMTF_CONJ ;
	m_iUpdateMethodW = iUpdateMW ;
	m_fThred_dist = fThred_dist/1000.0 ; // [mm] -> [m]
...
int Cexec::InitExportW(const char *cFilename,MICARY_PrmFrame *psPrmFrame)
{
	...
	if(cFilename){
		...
		for(ii = 0 ; ii < iNmic ; ii++){
			...
			// 2015/03/04 added by N.Tanaka #1823
			psDum->micary_locaMic.micary_Location[ii].tfposid = 
				m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[ii].tfposid ;
		}
		//周波数ライン
		for(ii = 0 ; ii < iNfreq ; ii++){
		...
		// TFDB中のPosID
		psDum->micary_locaSource.micary_Location[0].tfposid = 
				  m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[0].tfposid ;
				  
//cout << "Dummy PosID:" << psDum->micary_locaSource.micary_Location[0].tfposid << endl;	// TEST
		...
		if(m_iFormat_ExportW == __FILE_FORMAT_HARK){
			...
			// 2015/03/04 added by N.Tanaka 座標を伝達関数ファイルに対応したものに丸める
			for(ii = 0 ; ii < psH->lNSrc ; ii++){
				psDum->micary_locaSource.micary_Location[ii].ha_fX = 
					m_psTFDB->micary_locaSource.micary_Location[psDum->micary_locaSource.micary_Location[ii].tfposid].ha_fX ;
				psDum->micary_locaSource.micary_Location[ii].ha_fY = 
					m_psTFDB->micary_locaSource.micary_Location[psDum->micary_locaSource.micary_Location[ii].tfposid].ha_fY ;
				psDum->micary_locaSource.micary_Location[ii].ha_fZ = 
					m_psTFDB->micary_locaSource.micary_Location[psDum->micary_locaSource.micary_Location[ii].tfposid].ha_fZ ;
			}

//cout << "cHarkW.WriteData @ Cexec::InitExportW" << endl;	// TEST
			if(!cHarkW.WriteData(file.s,psH,
...
int Cexec::InitUpdateW()
{
	...
	for(ii = 0 ; ii < iNmic ; ii++){
		...
		// 2015/03/04 added by N.Tanaka #1823
		m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[ii].tfposid =
			m_psSepMat->micary_tdTFDB.micary_locaMic.micary_Location[ii].tfposid ;
	}
	...
	for(ii = 0 ; ii < iNLoc ; ii++){
		...
		// TFDB中の位置ID
		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid = m_psTFDB->micary_locaSource.micary_Location[ii].tfposid ;
	...
	//音源IDの初期化
	for(ii = 0 ; ii < iNLoc ; ii++){
		// 2014/12/1 changed by N.Tanaka 前詰めする
...
//		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_idPos = ii ;
		m_updateW->micary_dbSeparation.micary_locaSource.ha_idLocation[ii] = __INVALID_SRC_ID ;
		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_idPos = __INVALID_SRC_ID ;
		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid = __INVALID_SRC_ID ;
	}
	return 1 ;
...
int Cexec::ChangeSrcPos_TF_CONJ_W(MICARY_LocationArray plaSrcPosCur, MICARY_PrmFrame psPrmFrame)
{
	...
	for(n = 0 ; n < Npos_ini ; n++){
	   ...
	   for (int jj=0; jj < 3 ; jj++ ){
		...
		// TFDB中の位置ID	   
		m_plaSrcPosIni.micary_Location[n].tfposid = m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[n].tfposid ;
	}
	...
	// TEST
//	cout << "Input Pos: ";
//	for (i=0 ; i<plaSrcPosCur.ha_numLocation ; i++){
//		cout << plaSrcPosCur.micary_Location[i].tfposid << ", ";
//	}
//	cout << endl;
...
//cout << "@Cexec::ChangeSrcPos_TF_CONJ_W" << endl;

/*
int ifreqindex = 1;
for (k = 0 ; k < Npos_cur ; k++){
cout << "FD->G(f"<< ifreqindex <<"s"<< m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[k].tfposid <<"):";
for (m = 0 ; m < Nmic ; m++){
cout << "(" << m_psSepMat->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[k+m*Npos_cur+ifreqindex*Nmic*Npos_cur].re << ",";
cout << m_psSepMat->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[k+m*Npos_cur+ifreqindex*Nmic*Npos_cur].im << ")";
}
cout << "@Cexec::ChangeSrcPos_TF_CONJ_W" << endl;
}
*/
...
int Cexec::ChangeSrcPos_TF_CONJ(MICARY_LocationArray plaSrcPosCur, MICARY_PrmFrame psPrmFrame)
{
	...
	for(n = 0 ; n < Npos_ini ; n++){
	   ...
	   // TFDB中の位置ID
	   plaSrcPosIni.micary_Location[n].tfposid = m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[n].tfposid ;
	}
...
bool Cexec::_CheckSrcID_W(int iIdcur)
{
	...
	//分離行列の引継ぎ
		...
		// TFDB中の位置ID
		m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[iIdcur].tfposid 
			= m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[iIdsave].tfposid ;
...
//cout << "-EXIST" << iIDpos;
		...
		// TFDB中の位置ID	    
		m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[iIdcur].tfposid
			= m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[iIDpos].tfposid;
...
// ----------------------------------------------------------------------------------------
// 2点の位置がThreshold_lengthで設定された範囲内であるかの判定(Neighbor利用)
bool Cexec::_IsThreshPos(MICARY_Location laRefPos,MICARY_Location laObjPos)
{
	int i, iRefIndex, iObjIndex;
	
	if (!cHark.m_pPrinbrs){
		return (_IsThreshPosByCoord(laRefPos, laObjPos));
	}
	
	iRefIndex = cHark.GetPosIndex(laRefPos.ha_idPos);

	for (i=0 ; i < cHark.m_pPrinbrs->numNeighbors[iRefIndex] ; i++){
		if (cHark.m_pPrinbrs->neighbors[iRefIndex][i] == laObjPos.tfposid){
			return true;
		}
	}
	return false;
...
// ----------------------------------------------------------------------------------------
// 2点の位置がThreshold_lengthで設定された範囲内であるかの判定(座標計算)
bool Cexec::_IsThreshPosByCoord(MICARY_Location laRefPos,MICARY_Location laObjPos)
{
	HA_Float fdist = _EuclidDistance(laRefPos, laObjPos);
	return (fdist<m_fThred_dist);
}
// ----------------------------------------------------------------------------------------
...
bool Cexec::ExportUpdateW(MICARY_PrmFrame *psPrmFrame,const char *cFilename)
{
  int i,iNvalidSize,iNmic,iNfreq,iNpos,iiv,k,m ;
	...
	for(i = 0 ; i < iNmic ; i++){
		...
		// 2015/03/04 added by N.Tanaka #1823
		psValidW->micary_locaMic.micary_Location[i].tfposid = m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[i].tfposid;
	}
	...
	for(i = 0; i < iNpos ; i++){
		if(m_updateW->micary_iaUpdate.ha_iValue[i] == __SRCID_FRONT_CREATE || m_updateW->micary_iaUpdate.ha_iValue[i] == __SRCID_FRONT_EXIST){
			...
			// TFDB中の位置ID
			psValidW->micary_locaSource.micary_Location[iiv].tfposid = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[i].tfposid ;
	...
	if(m_iFormat_ExportW == __FILE_FORMAT_HARK){
		...
		// 2015/03/04 added by N.Tanaka 座標を伝達関数ファイルに対応したものに丸める
		for(i = 0 ; i < psH->lNSrc ; i++){
			psValidW->micary_locaSource.micary_Location[i].ha_fX = 
				m_psTFDB->micary_locaSource.micary_Location[psValidW->micary_locaSource.micary_Location[i].tfposid].ha_fX ;
			psValidW->micary_locaSource.micary_Location[i].ha_fY = 
				m_psTFDB->micary_locaSource.micary_Location[psValidW->micary_locaSource.micary_Location[i].tfposid].ha_fY ;
			psValidW->micary_locaSource.micary_Location[i].ha_fZ = 
				m_psTFDB->micary_locaSource.micary_Location[psValidW->micary_locaSource.micary_Location[i].tfposid].ha_fZ ;
		}

//cout << "cHarkW.WriteData @ Cexec::ExportUpdateW" << endl;	// TEST
		if(!cHarkW.WriteData(file.s,psH,
	...
	for(ii = 0 ; ii < Npos_cur ; ii++){
		...
		case __UPDATE_METHOD_POS :
			iPos = GetNearestPos(m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[ii],
				&(m_updateW->micary_dbSeparation.micary_locaSource), 1) ;
			if(iPos == __INVALID_SRC_ID){
			  for(jj = 0 ; jj < Npos_all ; jj++){
			    if(m_updateW->micary_iaUpdate.ha_iValue[jj] == __INVALID_SRC_ID){	// 新規に保存
					iPos = jj ;
					break ;
			    }
			  }
			...
			for (jj=0; jj < 3 ; jj++ ){
	    		...
	    		// TFDB中の位置ID
			m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[iPos].tfposid = m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[ii].tfposid ;
...
//			jj = GetNearestPos_ALLSearch(m_laFrontSrcLoc->micary_Location[ii],&m_psSepMat->micary_tdTFDB.micary_locaSource) ;
			// 2014/11/28 changed by N.Tanaka 1フレーム前に音源があってかつ，それが引き継ぎ範囲外であるかを確認(処理を整理)
/*
			jj = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_psSepMat->micary_tdTFDB.micary_locaSource) ;
			if(jj != __INVALID_SRC_ID){
				bRemove = _IsThreshPos(m_laFrontSrcLoc->micary_Location[ii],m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[jj]) ;	// ここはneighbor処理でよい
...
*/			
			jj = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_psSepMat->micary_tdTFDB.micary_locaSource, 1) ;
			if(jj == __INVALID_SRC_ID){ // 現フレームに前フレームの座標がない
				kk = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_updateW->micary_dbSeparation.micary_locaSource, 1) ; // m_UpdateW中のインデックスを検索
				if(kk!=__INVALID_SRC_ID){
//if (m_updateW->micary_iaUpdate.ha_iValue[kk] != __SRCID_FRONT_REMOVE){
//cout << "Src REMOVE" << kk << endl;
//}
					m_updateW->micary_iaUpdate.ha_iValue[kk] = __SRCID_FRONT_REMOVE;
					#ifdef __DEBUG_UPDATEW
					cout << "Src REMOVE " << endl ;
		...
		case __UPDATE_METHOD_ID_POS :
			...
			if(bRemove){
				// 2014/11/28 changed by N.Tanaka 前フレームの音源位置が現フレームの許容範囲にない
				/*
				jj = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_psSepMat->micary_tdTFDB.micary_locaSource) ;
				if(jj != __INVALID_SRC_ID){
				...
				*/
				jj = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_psSepMat->micary_tdTFDB.micary_locaSource, 1) ;
				if(jj != __INVALID_SRC_ID){	// 許容範囲内で見つかった
					bRemove = false;
				}
			}
			if(bRemove){	//消滅
				// 2014/12/11 changed by N.Tanaka
				// 現フレームに同じ音源位置があれば、m_Updateに保存される音源位置が複数にならないように__INVALID_SRC_IDを登録する
				/*
				kk = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_updateW->micary_dbSeparation.micary_locaSource, 1) ;
				if(kk!=__INVALID_SRC_ID){
					m_updateW->micary_iaUpdate.ha_iValue[kk] = __SRCID_FRONT_REMOVE;
				}
				*/
				int iUpdateType = __SRCID_FRONT_REMOVE;
				for (kk = ii+1 ; kk < m_psSepMat->micary_tdTFDB.micary_locaSource.ha_numLocation ; kk++){
					if (m_laFrontSrcLoc->micary_Location[kk].tfposid == m_laFrontSrcLoc->micary_Location[ii].tfposid){
						iUpdateType = __INVALID_SRC_ID;
						break;
					}
				}
				kk = GetNearestPos(m_laFrontSrcLoc->micary_Location[ii],&m_updateW->micary_dbSeparation.micary_locaSource, 1) ;
				if(kk!=__INVALID_SRC_ID){
					m_updateW->micary_iaUpdate.ha_iValue[kk] = iUpdateType;
					
					// 2014/12/17 added by N.Tanaka 座標も__INVALID_SRC_IDにしてく必要がある
					if (iUpdateType == __INVALID_SRC_ID){
						m_updateW->micary_dbSeparation.micary_locaSource.ha_idLocation[kk] = __INVALID_SRC_ID ;
						m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[kk].ha_idPos = __INVALID_SRC_ID ;
						m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[kk].tfposid = __INVALID_SRC_ID ;
					}
				}
			}
	...
	// 2014/12/11 added by N.Tanaka
	// ID_POSのとき、m_UpdateWに__SRCID_FRONT_EXISTとして存在している音源位置と同じものが__SRCID_FRONT_REMOVEとして存在している場合、__INVALID_SRC_IDに変更する
	for (ii = 0 ; ii < m_updateW->micary_dbSeparation.micary_locaSource.ha_numLocation ; ii++){
		if (m_updateW->micary_iaUpdate.ha_iValue[ii] != __SRCID_FRONT_EXIST){
			continue;
		}
		for (jj = 0 ; jj < m_updateW->micary_dbSeparation.micary_locaSource.ha_numLocation ; jj++){
			...
			if (m_updateW->micary_iaUpdate.ha_iValue[jj] != __SRCID_FRONT_REMOVE){
				continue;
			}
			if (m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid ==
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].tfposid){
				m_updateW->micary_iaUpdate.ha_iValue[jj] = __INVALID_SRC_ID;

				// 2014/12/17 added by N.Tanaka 座標も__INVALID_SRC_IDにしてく必要がある
				m_updateW->micary_dbSeparation.micary_locaSource.ha_idLocation[jj] = __INVALID_SRC_ID ;
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].ha_idPos = __INVALID_SRC_ID ;
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].tfposid = __INVALID_SRC_ID ;
			}
		}
	}
	...
	for(ii = 0 ; ii < m_laFrontSrcLoc->ha_numLocation ; ii++){
		...
		// TFDB中の位置ID		
		m_laFrontSrcLoc->micary_Location[ii].tfposid = m_psSepMat->micary_tdTFDB.micary_locaSource.micary_Location[ii].tfposid ;
...
//		ii = GetNearestPos_ALLSearch(lcCurLoc,m_laFrontSrcLoc) ;
		// 2014/11/28 changed by N.Tanaka ここは，音源があるかどうかが重要なので近くない値でも拾わなければならない処理を整理
/*
		...
		if(ii != __INVALID_SRC_ID){	//見つかった場合
			...
			if(btmp == true){	//許容範囲内(存在している)ならば，保存値データで検索
				...
				if(jj != __INVALID_SRC_ID){
					...
					// TFDB中の位置ID
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].tfposid = lcCurLoc.tfposid ;
		...
		return __INVALID_SRC_ID ;
*/
		jj = GetNearestPos(lcCurLoc,&m_updateW->micary_dbSeparation.micary_locaSource) ;
		if(jj != __INVALID_SRC_ID){	//見つかった場合
			//許容範囲内(存在している)ならば，保存値データで検索
			// 1フレーム前の音源位置で検索
			if(jj != __INVALID_SRC_ID){
				m_updateW->micary_iaUpdate.ha_iValue[jj] = __SRCID_FRONT_EXIST ;	// 復活したのか前フレームにあったのかは不明
				//保存値ﾃﾞｰﾀを現フレーム音源位置に更新
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].ha_fX = lcCurLoc.ha_fX ;
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].ha_fY = lcCurLoc.ha_fY ;
				m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].ha_fZ = lcCurLoc.ha_fZ ;
		...
		if(m_updateW == __UPDATE_METHOD_ID){	//add 2009.7.29
			...
			for(jj = 0 ; jj < m_updateW->micary_dbSeparation.micary_locaSource.ha_numLocation ; jj++){
				if(m_updateW->micary_iaUpdate.ha_iValue[jj] == __INVALID_SRC_ID){
					...
					// TFDB中の位置ID
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[jj].tfposid = lcCurLoc.tfposid ;
//cout << "source CREATE:" << jj << endl; // 2014/11/28 added by N.Tanaka
		...
		if(m_updateW->micary_iaUpdate.ha_iValue[ii] == __SRCID_FRONT_REMOVE && 
		 ...
		 if(!bExpW){
		   ...
		   cout << "Export file : " << cFilename <<" src = " << m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_idPos << endl ;
			...
			if(m_iFormat_ExportW == __FILE_FORMAT_HARK){
				if(!cHarkW.ReadHeader(file.s,psH)) //ファイルのヘッダ情報を取得
				{
				...
				// Read Header
				if(!cHarkW.ReadInfo(file.s,psH ,&pstmpW->micary_locaSource,
				...
				if(!cHarkW.ReadData(file.s,&pstmpW->micary_smSeparation.micary_cmpaSeparation)){
				...
				// 2014/8/27 deleted by N.Tanaka Neighbor利用のため付加情報不要
				/*
			...
			if(idtmp != __INVALID_SRC_ID){
				...
				//分離行列の引継ぎ
					...
					// TFDB中の位置ID
			  		pstmpW->micary_locaSource.micary_Location[idtmp].tfposid = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid ;
			  		bUpdate = true ;
				}
			}
...
//cout << "cHarkW.WriteData @ Cexec::AddUpdateW" << endl;	// TEST
					if(!cHarkW.WriteData(file.s,psH,
				...
				for(jj = 0 ; jj < Npos ; jj++){
					...
					// TFDB中の位置ID
					pstmpW2->micary_locaSource.micary_Location[jj].tfposid = 
						pstmpW->micary_locaSource.micary_Location[jj].tfposid ;
				...
				// TFDB中の位置ID
				pstmpW2->micary_locaSource.micary_Location[Npos].tfposid = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid ;
...
//cout << "cHarkW.WriteData 2 @ Cexec::AddUpdateW" << endl;	// TEST
				  if(!cHarkW.WriteData(file.s,psH,
...
#endif
//--------------------------------------------------------------------------
// 2014/11/26 changed by N.Tanaka libharkio3となったとき、保存は一回で済ますことが望ましくなったため変更
bool Cexec::AddUpdateW(MICARY_PrmFrame *psPrmFrame,const char *cFilename,bool bExpW)
{
	int ii ,jj ,Npos,Nmic,Nfreq,k,m,Npos_all,idtmp;
	bool bUpdate ;
	...
	MICARY_TFDBFileHeader * psH ;	//データベースヘッダ
	MICARY_SMDatabase *pstmpW, *pstmpW2 ;
	MICARY_SMDatabase *pstmpWTargetPtr, *pstmpWLatestPtr ;
	int iCurrentExpTarget;	// 1->pstmpW, 2->pstmpW2を保存
	HA_String file ; // ファイル名
	...
	if (bExpW){
		if(m_iFormat_ExportW == __FILE_FORMAT_HARK){
			...
			// 2014/11/28 deleted by N.Tanaka ダミーデータは必要なし
			/*
			if(!cHarkW.ReadHeader(file.s,psH)) //ファイルのヘッダ情報を取得
			{
				cout << "Can not read export file."<< endl ;
	...
	for(ii = 0 ; ii < Npos_all ;ii++){
		if(m_updateW->micary_iaUpdate.ha_iValue[ii] == __SRCID_FRONT_REMOVE && 
		 ...
		 if(!bExpW){
		    	//フラグの更新
		   // 2014/12/1 changed by N.Tanaka ID_POSの場合、POSでの分離行列情報を後で使用する場合がある
		   //if(m_iUpdateMethodW == __UPDATE_METHOD_ID || m_iUpdateMethodW == __UPDATE_METHOD_ID_POS){
		   if(m_iUpdateMethodW == __UPDATE_METHOD_ID){
			       m_updateW->micary_iaUpdate.ha_iValue[ii]=__INVALID_SRC_ID;
//cout << "RESET to __INVALID_SRC_ID" << ii << endl;
			    for(k = 0 ; k < Nfreq ; k++){
				 	for(m = 0 ; m < Nmic ; m++){
				    		m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[ii+m*Npos_all+k*Nmic*Npos_all].re = 0.0 ;
				    		m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[ii+m*Npos_all+k*Nmic*Npos_all].im = 0.0;
...
//		   	cout << "Export src = " << m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid << endl ;
			
			// 同じIDがあれば、更新
			// ポジションで検索してファイルデータベース更新ではないか？？
			bUpdate = false ;

			if (iCurrentExpTarget == 1){
				pstmpWTargetPtr = pstmpW;
			}
			else if (iCurrentExpTarget == 2){
				pstmpWTargetPtr = pstmpW2;
			}

			// 2014/12/11 changed by N.Tanak m_UpdateWにあるもの全てを保存するので、許容範囲内判定はしない
//		      	idtmp = GetNearestPos(m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii],&pstmpWTargetPtr->micary_locaSource) ;
		      	idtmp = GetNearestPos(m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii],&pstmpWTargetPtr->micary_locaSource, 1) ;
			if(idtmp != __INVALID_SRC_ID){	// 許容範囲内
//cout << "Update Process" << endl;
				...
				//分離行列の引継ぎ
		  		for(k = 0 ; k < Nfreq ; k++){
		    			for(m = 0 ; m < Nmic ; m++){
		      			pstmpWTargetPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[idtmp+m*Npos+k*Nmic*Npos].re 
									= m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[ii+m*Npos_all+k*Nmic*Npos_all].re ;
	   	      			pstmpWTargetPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[idtmp+m*Npos+k*Nmic*Npos].im 
									= m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[ii+m*Npos_all+k*Nmic*Npos_all].im ;
	   	    			}
		  		}
		  		//SrcPosも更新
		  		pstmpWTargetPtr->micary_locaSource.micary_Location[idtmp].ha_fX = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fX ;
		  		pstmpWTargetPtr->micary_locaSource.micary_Location[idtmp].ha_fY = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fY ;
		  		pstmpWTargetPtr->micary_locaSource.micary_Location[idtmp].ha_fZ = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fZ ;
				// TFDB中の位置ID
		  		pstmpWTargetPtr->micary_locaSource.micary_Location[idtmp].tfposid = m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid ;
		  		bUpdate = true ;
			}
...
//cout << "Add Process" << endl;
				...
				if (iCurrentExpTarget == 1){
					...
					SMDatabaseAlloc(pstmpW2,Nfreq,Npos+1,Nmic) ;
					pstmpWTargetPtr = pstmpW2;
					pstmpWLatestPtr = pstmpW;
					iCurrentExpTarget = 2;
				}
				else if (iCurrentExpTarget == 2){
					SMDatabaseFree(pstmpW) ;
					SMDatabaseAlloc(pstmpW,0,0,0) ;
					SMDatabaseAlloc(pstmpW,Nfreq,Npos+1,Nmic) ;
					pstmpWTargetPtr = pstmpW;
					pstmpWLatestPtr = pstmpW2;
					iCurrentExpTarget = 1;
				}

				//マイク座標
				for(jj = 0 ; jj < Nmic ; jj++){
					pstmpWTargetPtr->micary_locaMic.ha_idLocation[jj] = 
						m_updateW->micary_dbSeparation.micary_locaMic.ha_idLocation[jj] ;
					pstmpWTargetPtr->micary_locaMic.micary_Location[jj].ha_fX = 
						m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[jj].ha_fX ;
					pstmpWTargetPtr->micary_locaMic.micary_Location[jj].ha_fY = 
						m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[jj].ha_fY ;
					pstmpWTargetPtr->micary_locaMic.micary_Location[jj].ha_fZ = 
						m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[jj].ha_fZ ;
					pstmpWTargetPtr->micary_locaMic.micary_Location[jj].ha_idPos = 
						m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[jj].ha_idPos ;

					// 2015/03/04 added by N.Tanaka #1823
					pstmpWTargetPtr->micary_locaMic.micary_Location[jj].tfposid = 
						m_updateW->micary_dbSeparation.micary_locaMic.micary_Location[jj].tfposid ;
				}

				//周波数ライン
				for(jj = 0 ; jj < Nfreq ; jj++){
					pstmpWTargetPtr->micary_smSeparation.micary_iaFreqLinex.ha_iValue[jj] = 
						m_updateW->micary_dbSeparation.micary_smSeparation.micary_iaFreqLinex.ha_iValue[jj] ;
				}

				//音源座標
				for(jj = 0 ; jj < Npos ; jj++){
					pstmpWTargetPtr->micary_locaSource.ha_idLocation[jj] = 
						pstmpWLatestPtr->micary_locaSource.ha_idLocation[jj] ;
					pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fX = 
						pstmpWLatestPtr->micary_locaSource.micary_Location[jj].ha_fX ;
					pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fY = 
						pstmpWLatestPtr->micary_locaSource.micary_Location[jj].ha_fY ;
					pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fZ = 
						pstmpWLatestPtr->micary_locaSource.micary_Location[jj].ha_fZ ;
					pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_idPos = 
						pstmpWLatestPtr->micary_locaSource.micary_Location[jj].ha_idPos ;
					
					// TFDB中の位置ID
					pstmpWTargetPtr->micary_locaSource.micary_Location[jj].tfposid = 
						pstmpWLatestPtr->micary_locaSource.micary_Location[jj].tfposid ;
					

				}
	
				pstmpWTargetPtr->micary_locaSource.ha_idLocation[Npos] = 
					m_updateW->micary_dbSeparation.micary_locaSource.ha_idLocation[ii] ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[Npos].ha_fX = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fX ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[Npos].ha_fY = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fY ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[Npos].ha_fZ = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fZ ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[Npos].ha_idPos = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_idPos ;

				// TFDB中の位置ID
				pstmpWTargetPtr->micary_locaSource.micary_Location[Npos].tfposid = 
					m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid ;
				...
				*/
				...
				for(jj = 0 ; jj < Npos +1 ; jj++){								
					for(k = 0 ; k < Nfreq ; k++){
						for(m = 0 ; m < Nmic ; m++){
							if(jj < Npos){
								pstmpWTargetPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[jj+m*(Npos+1)+k*Nmic*(Npos+1)].re 
									= pstmpWLatestPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[jj+m*Npos+k*Nmic*Npos].re ;
		   						pstmpWTargetPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[jj+m*(Npos+1)+k*Nmic*(Npos+1)].im 
									= pstmpWLatestPtr->micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[jj+m*Npos+k*Nmic*Npos].im ;
	...
	if (bExpW){
		if(m_iFormat_ExportW == __FILE_FORMAT_HARK){
		   	cout << "Export file : " << cFilename << endl ;
			...
			psH->lNFFT = psPrmFrame->ha_numNFFT ;
			// 2010.8.11
			psH->lNSrc = pstmpWTargetPtr->micary_locaSource.ha_numLocation ;
			psH->lNMic =  pstmpWTargetPtr->micary_locaMic.ha_numLocation ;
			psH->lNFreq = pstmpWTargetPtr->micary_smSeparation.micary_iaFreqLinex.ha_numValue ;
			psH->iVersion = 1 ;
			psH->iFileType = FORMAT_TYPE_W ;

			// 2015/03/04 added by N.Tanaka 座標を伝達関数ファイルに対応したものに丸める
			for(jj = 0 ; jj < psH->lNSrc ; jj++){
				pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fX = 
					m_psTFDB->micary_locaSource.micary_Location[pstmpWTargetPtr->micary_locaSource.micary_Location[jj].tfposid].ha_fX ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fY = 
					m_psTFDB->micary_locaSource.micary_Location[pstmpWTargetPtr->micary_locaSource.micary_Location[jj].tfposid].ha_fY ;
				pstmpWTargetPtr->micary_locaSource.micary_Location[jj].ha_fZ = 
					m_psTFDB->micary_locaSource.micary_Location[pstmpWTargetPtr->micary_locaSource.micary_Location[jj].tfposid].ha_fZ ;
			}
			...
			*/
			...
			if(!cHarkW.WriteData(file.s,psH,
					pstmpWTargetPtr->micary_smSeparation.micary_cmpaSeparation,
					pstmpWTargetPtr->micary_smSeparation.micary_iaFreqLinex,
					pstmpWTargetPtr->micary_locaSource,
					pstmpWTargetPtr->micary_locaMic,
					"",
					"",
					"",
					"",
...
/*
int ifreqindex = 1;
for (k = 0 ; k < Npos_all ; k++){
if (m_updateW->micary_iaUpdate.ha_iValue[k] != __INVALID_SRC_ID){
cout << "FD->SepDB(f"<< ifreqindex <<"s"<< m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[k].tfposid <<"):";
for (m = 0 ; m < Nmic ; m++){
cout << "(" << m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[k+m*Npos_all+ifreqindex*Nmic*Npos_all].re << ",";
cout << m_updateW->micary_dbSeparation.micary_smSeparation.micary_cmpaSeparation.ha_cmpValue[k+m*Npos_all+ifreqindex*Nmic*Npos_all].im << ")";
}
cout << "@Cexec::AddUpdateW" << endl;
}
}
*/
...
//cout << "m_updateW->micary_iaUpdate.ha_iValue" << endl;// 2014/11/28 TEST
	for(ii = 0 ; ii < Npos_all ;ii++){
//cout << ii << "->update" << m_updateW->micary_iaUpdate.ha_iValue[ii] << ", tfposid" << m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].tfposid << endl;// 2014/11/28 TEST
...
/*
cout << "<hark-fd>m_UpdateW:";
for (int k = 0 ; k < Npos_all ; k++){
if (m_updateW->micary_iaUpdate.ha_iValue[k] != __INVALID_SRC_ID){
cout << m_updateW->micary_iaUpdate.ha_iValue[k] << "-" << m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[k].tfposid << ",";
}
}
cout << "@Cexec::ExportAllUpdateW(1)" << endl;
*/
...
cout << "@Cexec::ExportAllUpdateW(2)" << endl;
*/
	return bRtn ;
}
